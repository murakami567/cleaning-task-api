...
def create_room(property_id:str=Body(...), room_name:str=Body(...), room_code:str|None=Body(None), room_key:str|None=Body(None), normalized_room_key:str|None=Body(None), capacity:int|None=Body(1), room_sort_order:int|None=Body(999), is_active:bool=Body(True), prep_d:int|None=Body(0), prep_s:int|None=Body(0), prep_spare_s:int|None=Body(0), prep_ta:int|None=Body(0), cleaning_score:int|None=Body(None), keybox_number:str|None=Body(None), spare_key_number:str|None=Body(None), mailbox_number:str|None=Body(None), wifi_ssid:str|None=Body(None), wifi_password:str|None=Body(None), note:str|None=Body(None), early_checkin_fee:int|None=Body(0), late_checkout_fee:int|None=Body(0), current_user:dict=Depends(require_admin_write)):
    property_id=_text(property_id); room_name=_text(room_name)
    if not property_id: raise HTTPException(status_code=400,detail="property_id is required")
    if not room_name: raise HTTPException(status_code=400,detail="room_name is required")
    code=_text(room_code) or room_name; key=_text(room_key) or room_name; normalized_key=_text(normalized_room_key) or key
    payload={"property_id":property_id,"room_name":room_name,"room_code":code,"room_key":key,"normalized_room_key":normalized_key,"capacity":_int_or_default(capacity,1),"room_sort_order":_int_or_default(room_sort_order,999),"is_active":is_active,"prep_d":_int_or_default(prep_d,0),"prep_s":_int_or_default(prep_s,0),"prep_spare_s":_int_or_default(prep_spare_s,0),"prep_ta":_int_or_default(prep_ta,0),"keybox_number":_text(keybox_number),"spare_key_number":_text(spare_key_number),"mailbox_number":_text(mailbox_number),"wifi_ssid":_text(wifi_ssid),"wifi_password":_text(wifi_password),"note":_text(note),"early_checkin_fee":_int_or_default(early_checkin_fee,0),"late_checkout_fee":_int_or_default(late_checkout_fee,0)}
    if cleaning_score is not None: payload["cleaning_score"]=_score(cleaning_score)
    try: res=supabase.table("rooms").insert(payload).execute()
...
def bulk_create_rooms(property_id:str=Body(...), room_names:list[str]=Body(...), default_capacity:int|None=Body(1), start_sort_order:int|None=Body(1), current_user:dict=Depends(require_admin_write)):
    property_id=_text(property_id); names=[_text(n) for n in room_names or [] if _text(n)]
    if not property_id: raise HTTPException(status_code=400,detail="property_id is required")
    if not names: raise HTTPException(status_code=400,detail="room_names is required")
    try: prop_res=supabase.table("properties").select("property_name").eq("id",property_id).limit(1).execute()
    except Exception as e:
        logger.error(f"bulk_create_rooms property lookup failed: property_id={property_id} {e}",exc_info=True); raise HTTPException(status_code=500,detail="property lookup failed")
    property_name=(prop_res.data or [{}])[0].get("property_name") if prop_res.data else ""; base_order=_int_or_default(start_sort_order,1); cap=_int_or_default(default_capacity,1)
    rows=[]; seen=set()
    for index,name in enumerate(names):
        if name in seen: continue
        seen.add(name); rk=f"{property_name}{name}" if property_name else name
        rows.append({"property_id":property_id,"room_name":name,"room_code":name,"room_key":rk,"normalized_room_key":rk,"capacity":cap,"room_sort_order":base_order+index,"is_active":True,"prep_d":0,"prep_s":0,"prep_spare_s":0,"prep_ta":0,"keybox_number":"","spare_key_number":"","mailbox_number":"","wifi_ssid":"","wifi_password":"","note":""})
...
def delete_room(room_id:str=Body(...,embed=True), current_user:dict=Depends(require_admin_write)):
