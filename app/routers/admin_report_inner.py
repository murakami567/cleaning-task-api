...
from app.services.auth_service import require_admin_or_leader, require_worklog_write
...
def update_admin_worklog(payload: WorklogUpdateBody, current_user: dict = Depends(require_worklog_write)):
...
def delete_admin_worklog(payload: WorklogDeleteBody, current_user: dict = Depends(require_worklog_write)):
    if not payload.worklog_id:
        raise HTTPException(status_code=400, detail="worklog_id is required")
