...
from app.services.auth_service import require_operational_write
...
def create_shift_day(shift_date: str = Body(...), note: str = Body(""), current_user: dict = Depends(require_operational_write)):
...
def get_or_create_shift_day(shift_date: str = Body(...), note: str = Body(""), current_user: dict = Depends(require_operational_write)):
    try:
        existing = (
            supabase.table("shift_days")
            .select("*")
...
def upsert_staff_schedule(body: dict[str, Any] = Body(...), current_user: dict = Depends(require_operational_write)):
