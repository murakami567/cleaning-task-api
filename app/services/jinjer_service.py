...
def _fetch_attendances_one_day(target: str, safety_max_pages: int) -> list[dict[str, Any]]:
    items: list[dict[str, Any]] = []
    page = 1
    ...
    while page <= safety_max_pages:
        ...
        try:
            ...
            logger.error(f"jinjer attendances request failed: date={target} page={page} params={params} {e}", exc_info=True)
            raise HTTPException(status_code=502, detail="Jinjer 打刻取得に失敗しました。")

        if res.status_code != 200:
            logger.error(
                f"jinjer attendances returned {res.status_code}: endpoint={JINJER_ATTENDANCE_ENDPOINT} date={target} response={res.text[:700]}"
            )
    ...
    logger.info(f"jinjer attendances day fetched: date={target} pages={page} items={len(items)}")
    return items
...
def fetch_attendances(start_date: str, end_date: str | None = None) -> list[dict[str, Any]]:
    ...
    while current <= end_d:
        ...
        day_items = _fetch_attendances_one_day(target, safety_max_pages)
        for normalized in day_items:
