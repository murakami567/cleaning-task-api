...
OPERATIONAL_FIELDS = (
    ...
    "assignment_locked", "cleaning_started_at",
)
...
def find_target_columns(header_row):
    target_columns = {
        "Title": -1, "Property": -1, "Unit": -1, "FirstNight": -1,
        "Check Out": -1, "Price": -1, "Status": -1, "Referer": -1,
        "Adult": -1, "Child": -1, "Time Entered": -1, "Full Name": -1, "Ref": -1,
    }
...
def split_property_and_room(property_raw: str, unit_raw: str):
    ...
    if not room_name and raw_property.startswith(normalized_property):
        room_name = raw_property[len(normalized_property):].strip()
...
def calc_gap_nights(checkout_date: str | None, next_checkin_date: str | None):
    if not checkout_date or not next_checkin_date:
        return 0
    try:
        return max((datetime.fromisoformat(next_checkin_date).date() - datetime.fromisoformat(checkout_date).date()).days, 0)
    except Exception:
        return 0
...
def calc_stay_nights(checkin_date: str | None, checkout_date: str | None):
    if not checkin_date or not checkout_date:
        return 0
    try:
        return max((datetime.fromisoformat(checkout_date).date() - datetime.fromisoformat(checkin_date).date()).days, 0)
    except Exception:
        return 0
...
def append_sync_alert(existing_note, old_next_checkin, new_next_checkin, old_task_date):
...
def _has_assignment(row: dict) -> bool:
    return bool(row.get("assigned_staff_id") or row.get("assigned_staff_ids") or row.get("assigned_staff_name") or row.get("assigned_staff_names"))


def _status_priority(status):
    return {"持越": 60, "清掃中": 50, "清掃開始": 45, "清掃完了": 40, "完了": 40, "CXL": 10, "未着手": 20}.get(str(status or "").strip(), 0)
...
def _keeper_score(row, booking_id):
    return (
        _status_priority(row.get("status")), 1 if _has_assignment(row) else 0,
...
def _dedupe_rows(rows):
    ...
    for row in rows:
        ...
        key = row_id or (str(row.get("booking_id") or ""), str(row.get("room_key") or ""), str(row.get("checkout_date") or ""))
        if key in seen:
            continue
        seen.add(key)
        result.append(row)
...
def find_existing_candidates(payload):
    candidates = []
    booking_id = str(payload.get("booking_id") or "").strip()
    if booking_id:
        res = supabase.table("cleaning_tasks").select("*").eq("booking_id", booking_id).execute()
    ...
    if room_key and checkout_date:
        res = supabase.table("cleaning_tasks").select("*").eq("room_key", room_key).eq("checkout_date", checkout_date).execute()
        candidates.extend(res.data or [])
    return _dedupe_rows(candidates)


def preserve_operational_fields(payload, existing):
    if not existing:
        return payload
...
def apply_carry_over_safety(payload, existing):
    ...
    if carry_date <= next_checkin_date:
        ...
        return payload, None
    payload.update({
        "task_date": new_next_checkin, "status": CARRY_OVER_RESET_STATUS,
        "assigned_staff_ids": [], "assigned_staff_names": [], "assigned_staff_id": None,
        "assigned_staff_name": None, "checker_id": None, "checker_name": None,
        "assignment_locked": False, "cleaning_started_at": None,
        "note": append_sync_alert(existing.get("note"), old_next_checkin, new_next_checkin, old_task_date),
    })
    return payload, {
        "booking_id": payload.get("booking_id"), "property_name": payload.get("property_name"),
        "room_name": payload.get("room_name"), "old_task_date": old_task_date,
        "new_task_date": new_next_checkin, "old_next_checkin_date": old_next_checkin,
        "new_next_checkin_date": new_next_checkin, "reason": "carry_over_exceeded_new_next_checkin",
    }


def save_canonical_cleaning_task(payload):
    candidates = find_existing_candidates(payload)
    if candidates:
        booking_id = str(payload.get("booking_id") or "")
        ordered = sorted(candidates, key=lambda row: _keeper_score(row, booking_id), reverse=True)
        ...
        update_res = supabase.table("cleaning_tasks").update(payload).eq("id", keeper["id"]).execute()
        removed = []
        for duplicate in duplicates:
            duplicate_id = duplicate.get("id")
            if duplicate_id:
                supabase.table("cleaning_tasks").delete().eq("id", duplicate_id).execute()
                removed.append({"id": duplicate_id, "booking_id": duplicate.get("booking_id"), "status": duplicate.get("status")})
        return {"mode": "updated", "saved_count": len(update_res.data or []), "keeper_id": keeper.get("id"), "adjustment": adjustment, "removed_duplicates": removed}
    insert_res = supabase.table("cleaning_tasks").insert(payload).execute()
    inserted = (insert_res.data or [None])[0]
    return {"mode": "inserted", "saved_count": len(insert_res.data or []), "keeper_id": inserted.get("id") if inserted else None, "adjustment": None, "removed_duplicates": []}


def cancel_existing_block_task(booking_id: str, title: str):
    """ブロック/予備部屋は新規作成せず、同じ予約IDの既存タスクだけCXLへ変更する。"""
    existing_res = supabase.table("cleaning_tasks").select("id").eq("booking_id", booking_id).execute()
...
def beds24_csv_sync_service(from_date: str | None = None, to_date: str | None = None):
    ...
    start_date_str = from_date if from_date and from_date != "string" else format_date_string(start)
    end_date_str = to_date if to_date and to_date != "string" else format_date_string(end)
    request_payload = {"username": BEDS24_CSV_USERNAME, "password": BEDS24_CSV_PASSWORD, "datefrom": start_date_str, "dateto": end_date_str}
    ...
    if not csv_rows:
        return {"ok": True, "from": start_date_str, "to": end_date_str, "csv_row_count": 0, "cleaning_saved_count": 0, "carry_over_adjusted_count": 0, "duplicate_removed_count": 0, "skipped_count": 0, "cleaning_saved": [], "carry_over_adjusted": [], "duplicates_removed": [], "skipped": []}

    target_columns = find_target_columns(csv_rows[0])
    cleaning_saved, carry_over_adjusted, duplicates_removed, skipped = [], [], [], []
    records = []
    ...
    for i in range(1, len(csv_rows)):
        ...
        try:
            ...
            if "ブロック" in title or "予備部屋" in title:
                updated_count = cancel_existing_block_task(ref_raw, title)
                skipped.append({
                    "reason": "blocked_existing_task_cancelled" if updated_count else "blocked_no_existing_task",
                    "row_index": i, "booking_id": ref_raw,
                    "beds24_title": str(title or "").strip(), "updated_count": updated_count,
                })
                continue
    ...
    for room_key, room_records in grouped.items():
        ...
        for idx, rec in enumerate(room_records):
            next_checkin_date, next_guest_count, next_stay_nights = None, 0, 0
            if idx + 1 < len(room_records):
                ...
                next_stay_nights = calc_stay_nights(next_rec["checkin_date"], next_rec["checkout_date"])
            gap_nights = calc_gap_nights(rec["checkout_date"], next_checkin_date)
            final_records.append({
                "booking_id": rec["booking_id"],
                "beds24_title": rec.get("beds24_title") or "",
                "property_name": rec["property_name"], "room_name": rec["room_name"], "room_key": rec["room_key"],
                "task_date": rec["checkout_date"], "checkout_date": rec["checkout_date"], "next_checkin_date": next_checkin_date,
                "gap_nights": gap_nights, "guest_count": rec["guest_count"], "next_guest_count": next_guest_count,
                "next_stay_nights": next_stay_nights, "load_score": calc_load_score(rec["guest_count"], gap_nights),
                "status": "未着手", "note": "", "source": "beds24_csv",
            })

    for payload in final_records:
        try:
            result = save_canonical_cleaning_task(payload)
            cleaning_saved.append({"booking_id": payload["booking_id"], "mode": result["mode"], "keeper_id": result["keeper_id"], "count": result["saved_count"]})
            if result["adjustment"]:
                carry_over_adjusted.append(result["adjustment"])
            if result["removed_duplicates"]:
                duplicates_removed.append({"room_key": payload["room_key"], "checkout_date": payload["checkout_date"], "keeper_id": result["keeper_id"], "removed": result["removed_duplicates"]})
        except Exception as e:
            skipped.append({"reason": f"save error: {str(e)}", "booking_id": payload["booking_id"], "room_key": payload.get("room_key"), "checkout_date": payload.get("checkout_date")})

    duplicate_removed_count = sum(len(item.get("removed") or []) for item in duplicates_removed)
    return {
        "ok": True, "from": start_date_str, "to": end_date_str,
        "csv_row_count": len(csv_rows) - 1, "cleaning_saved_count": len(cleaning_saved),
        "carry_over_adjusted_count": len(carry_over_adjusted), "duplicate_removed_count": duplicate_removed_count,
        "skipped_count": len(skipped), "cleaning_saved": cleaning_saved[:20],
        "carry_over_adjusted": carry_over_adjusted[:50], "duplicates_removed": duplicates_removed[:50], "skipped": skipped[:50],
    }
